...
import { emittedEvents } from '../dist/index.js'
...
| Name | Actions |
| ---- | ---- |
${Object.keys(emittedEvents).map(key => {
    const actions = emittedEvents[key]

    return `| ${key} | ${actions.length > 0 ? actions.join(', ')  :  '*'} |`
}).join('\n')}
`

writeFileSync(resolve('.', './docs/events.md'), content)